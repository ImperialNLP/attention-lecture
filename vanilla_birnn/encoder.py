...
class Encoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        _, final_backwards = self.backwards_rnn(reverse_embedded, zeros)
        # shape(final_backwards) = [1, B, D]

        # > ensure that final_forward and final_backwards are [B, D]
        final_forwards = final_forwards.squeeze(0)
        final_backwards = final_backwards.squeeze(0)

        # > concatenate final_forward and final_backwards
        output = torch.cat((final_forwards, final_backwards), dim=1)
        # shape(output) = [B, 2D]
